...
CONFIG = {
    ...
    #   ★根拠（2026-09-12）＝この1,500円は「会社が一番損をしない限界値」＝
    #       (送料＋両店の手間) ÷ 原価率0.89。
    #     ・原価率0.89＝仕入単価÷薬価。4店の薬VAN全在庫の実測（在庫金額で加重した平均）。
    #       捨てたときに消える損の実額は「薬価」ではなく「仕入値＝薬価×0.89」なので、
    #       送って取り戻せるのもこの仕入値。送った方が得＝薬価金額×0.89 ＞ 送料＋手間。
    #     ・ネコポス420円＋手間20〜30分（時給1,700円換算 567〜850円）＝987〜1,270円。
    #       これを÷0.89すると 1,110〜1,430円 → 安全側へ切り上げて 1,500円。
    #   ★次に送料・送り方が変わったらこの1つの数字を直すだけで、板と月次リスト②③④に同時に効く。
    #     例）ゆうパックが標準になったら 2000〜2500 に上げる（限界値＝(約1,000円＋手間)÷0.89）。
