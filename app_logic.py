...
def build_allboard_tree(rows):
    ...
    # 並び順（本間部長決定・2026-08-22）：
    #   ・大元（ルート）＝新しい順（新しい投稿が一番上）。蓄積しても新着を下まで探しに行かずに済む。
    #   ・返信（子）＝今までどおり古い→新しい順のまま。★ここを逆にすると
    #     「引き取ります→伺います→ありがとう」が逆さに並んで会話が読めなくなるので、絶対に変えない。
    # 同時刻でも並びが安定するよう、今までどおり投稿IDを第2キーにする（第2キーの作りは壊さない）。
    def _sort_key(x):
        return (str(x.get('投稿日時', '') or ''), str(x.get('投稿ID', '') or ''))
    # ★roots だけ reverse=True で降順にする。第1キー（投稿日時）が新しい順に並ぶ。
    #   ★reverse=True は第2キー（投稿ID）も降順にするが、投稿IDは実質ユニークなので
    #     同時刻でも並びは一意に定まる＝毎回同じ（安定）。向きが逆でも取り違えは起きない。
    roots.sort(key=_sort_key, reverse=True)
    # ★children は reverse を付けない＝昇順（古い→新しい）のまま。返信の会話順を守る。
    for k in children:
